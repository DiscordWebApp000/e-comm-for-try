...
const ProductPage = ({ params }) => {
  ...
  // Redux state selectors to get product details and authentication state
  ...
  // Fetch product details when slug is available
  useEffect(() => {
    if (slug) {
            ...
            {token ? (
              <div className="bg-gray-100 p-4 rounded-md shadow-sm">
                ...
                <input
                  ...
                  id="rating"
                  value={userRating}
                  onChange={handleRatingChange} // Handle change in rating input
                  min="1"
                  max="5"
                  className="mt-2 w-full p-3 border-2 rounded-md border-gray-300 focus:outline-none focus:border-blue-500"
                />
                <button
                  onClick={handleSubmitRating} // Submit rating when button is clicked
